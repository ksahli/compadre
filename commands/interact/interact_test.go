// This test is in-package rather than in interact_test, for two reasons. What
...
// against a real API; and the session itself is unexported, being the piece of
// behaviour this package owns now that the loop lives in core.
package interact
...
func TestNew(t *testing.T) {
	cases := []struct {
		...
		{
			name:         "instructions to stand over the session",
		...
		{
			// A session picks an exchange up as readily as it
			// opens one: the turns still come from stdin, they
			// just carry on something already filed.
			name:      "an exchange to continue",
			arguments: []string{"-exchange", "7"},
			exchange:  "7",
		},
		{
			name:      "a model of the caller's choosing",
			arguments: []string{"-model", "claude-opus-5"},
			model:     "claude-opus-5",
		},
		{
			name:      "a ceiling on one reply",
			arguments: []string{"-max-tokens", "4096"},
			tokens:    4096,
		},
		{
			name:      "retries of a request the API turned away",
			arguments: []string{"-max-retries", "5"},
			retries:   5,
		},
		{
			name:      "a workspace somewhere other than here",
			arguments: []string{"-workspace", "/tmp"},
			directory: "/tmp",
		},
		{
			name:      "a ceiling on the turns",
			arguments: []string{"-max-turns", "25"},
			turns:     25,
		},
		{
		...
		},
		{
			...
			// session, and off unless it is asked for.
			name:      "what each turn cost, asked for",
			arguments: []string{"-usage"},
			usage:     true,
		},
	}
	for _, c := range cases {
...
// -message is not among them, which is the difference from invoke stated as a
// thing a caller can read.
func TestNewReportsHelpAsItsUsage(t *testing.T) {
	for _, argument := range []string{"-h", "-help"} {
		t.Run(argument, func(t *testing.T) {
			...
			for _, want := range []string{"-instructions", "-exchange", "-store", "-model", "-max-tokens", "-max-retries", "-workspace", "-max-turns", "-usage"} {
...
func TestNewRejectsUnparseableArguments(t *testing.T) {
	cases := []struct {
		...
		// -message belongs to invoke. Here it is a flag nothing
		// defines, which is the whole of how the split is enforced:
		// a caller with one thing to ask is told they want the other
		// command rather than quietly given a prompt.
		{"the flag the other command owns", []string{"-message", "hello"}},
		{"a flag missing its value", []string{"-instructions"}},
...
func TestExecuteRejectsInstructionsOnAResumedExchange(t *testing.T) {
	command := &Command{exchange: "7", instructions: "be brief"}
...
// conversation.
//
// No turn is folded in here, in either shape. The exchange is opened before
// the first line has been read, and every turn the session goes on to read
// folds itself on: that is what lets one function serve both shapes.
...
func (t *turning) Invoke(ctx context.Context, thread threads.Type, registry tools.Registry) ([]messages.Type, error) {
...
func (w *waiting) Invoke(ctx context.Context, thread threads.Type, registry tools.Registry) ([]messages.Type, error) {
	if w.entered == nil {
...
func (s *stumbling) Invoke(ctx context.Context, thread threads.Type, registry tools.Registry) ([]messages.Type, error) {
	s.calls++
	if s.calls == s.on {
...
func (wedged) Save(context.Context, Exchange) (Exchange, error) {
	return Exchange{}, errors.New("there is nowhere to write it")
}

func (wedged) Load(context.Context, string) (Exchange, error) {
	return Exchange{}, errors.New("not asked for here")
}
