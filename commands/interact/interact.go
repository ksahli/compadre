// Package interact holds an exchange with a model at a prompt, a turn at a
// time.
//
...
// take, reading the turns, and printing.
//
...
//
// The turns are read from stdin a line at a time and each is answered in the
// exchange the last one grew, in one process and under one id. It is the loop
// compadre invoke -exchange runs across processes, with the exchange never
// leaving rather than being read back out of the store between turns.
//
// An interrupt ends the turn it landed in and nothing else, which is why this
// package listens for it rather than the process it is a command of. What the
// model had said is already printed, the record already has everything that got
// that far, and the prompt comes back with the same exchange to carry on. Ctrl-C
// at a prompt with nothing in flight is a gesture at nothing, so the first is
// answered with a way out rather than acted on and a second one takes it — the
// session ends, the same as the end of input.
//
// A turn that failed is the same shape, and for the same reason. What went
// wrong is said at the prompt and the next turn is asked for, since a rate
// limit or a reply cut off at the ceiling says nothing about whether the turn
// after it can be answered. The failures that do say something are marked
...
// under that id, read back out of the store and continued — the same session
// either way, since the loop is handed an exchange rather than asked to start
// one, and a session picks one up as readily as it opens one.
//
...
// does the prompt, and so does what a turn cost when -usage asks for it,
// because all three are this program talking about itself rather than the
// answer to what was asked. That is what lets a session be driven by a pipe as
// well as by a person, with no test for which it is: the prompts go where the
// answer is not.
package interact
...
// Command is a parsed interact: the instructions to stand over the exchange,
// the exchange to continue if there is one, where to keep the record of it,
// what the session is bounded by — the model, the ceiling on one reply, the
// retries of a request the API turned away, the directory the file tools may
// see and the ceiling on the turns — whether what each turn cost is to be
// said, and the three streams it is reached through.
//
...
// bounds. It is not something the session is held to; it is a question about what
...
// one -exchange names. Then the session that reads the turns from stdin.
//
// One thing is settled before any of that. An exchange that already exists
// carries the instructions it was opened with, and they are not this session's
// to replace. Saying so beats taking the flag and ignoring it, which is the only
...
func (c *Command) Execute(ctx context.Context) error {
	...
	// The exchange comes back whether the session ended well or badly, so
	// what was said is printed either way. The same goes for where it was
	// filed — a session that ended on a settled failure still left a record,
	// and the id is how a reader finds it, which is what makes it something
	// to be picked back up rather than lost.
...
// grew. It is the same loop the command line runs across processes with
// invoke -exchange, with the store standing in for memory; here the exchange
// never leaves, so nothing is read back and the id is minted once.
//
// A blank line is not a turn. An exchange with nothing to carry it on is the
// mistake invoke refuses -message for, and at a prompt the kindest answer is to
...
// a session that gave up after -max-turns — each of those is a turn that did not
...
// failed turn is not.
func (c *Command) session(ctx context.Context, agent Agent, exchange Exchange, interrupts <-chan os.Signal) (Exchange, error) {
	lines := listen(c.in)
	armed := false
...
// open is the exchange this session is to carry on: the one -exchange names,
// or a new one opened with the instructions given here. Either way what comes
// back is an exchange a turn can be folded onto, which is why the difference
// between the two ends here rather than being carried any further in.
//
// No turn is folded in here, and there is none to fold: the first one has not
// been read yet, and an exchange with nothing in it is a perfectly good thing
// to hand it to when it arrives.
//
...
func read(ctx context.Context, turns <-chan turn, interrupts <-chan os.Signal) (turn, heard) {
	select {
	case next, ok := <-turns:
		if !ok {
...
// stream the answer is on: it is this program accounting for the session rather
...
// New parses the arguments interact understands: -instructions for the system
// instructions, -exchange for an exchange to continue rather than open, -store
// for where to keep the record of it, -model for the model to reach,
// -max-tokens for the ceiling on one reply it may write and -max-retries for
// how many times a request the API turned away is sent again, -workspace for
// the directory the file tools may see, and -max-turns for how many turns one
// exchange may take.
//
// There is no -message here, and that is the whole of what separates this
// command from invoke: the turns come from stdin, so there is nothing for a
// flag to say. A caller who has one thing to ask wants invoke.
//
// An absent -store is not a session that keeps no record: it is one that keeps
// its record where this program keeps them by default, and an absent -exchange
// is a session that opens one rather than one that keeps nothing. The same
// holds for the five bounds — an absent one of them is the value that was true
// before there was a flag to say otherwise, which is why the zero value stands
// for absence and the default is reached for at [Command.Execute]. An argument
// the flag set cannot make sense of comes back as an error: parsing is not the
// place to end the process, and the caller is already reporting.
//
...
func New(arguments []string) (*Command, error) {
	c, f := new(Command), flag.NewFlagSet("interact", flag.ContinueOnError)
	usage := &bytes.Buffer{}
	f.SetOutput(usage)
